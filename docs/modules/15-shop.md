...
`player-repository.ts` 的 `claimIBShopFreePack` / `IBShopError`、
`gateway-server.ts` 的 `parseDoRechargeCall` 与分发分支。

## 验证

`test/shop-data.test.ts` 验证空商品列表的字段和值；
`test/ib-shop-persistence.test.ts` 覆盖发货+计数、当日限购拒绝、跨运营日重置、
未知账号。没有设备购买流程或网关集成测试。
